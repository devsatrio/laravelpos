                        ...
                        <form method="POST" onsubmit="return validasiinput();" role="form" enctype="multipart/form-data"
                            ...
                            <div class="card-body">
                                ...
                                <div class="row">
                                    ...
                                    @foreach ($permission_grub as $row_permission_grub)
                                        <div class="col-md-4">
                                            <div class="card card-secondary">
                                                <div class="card-header">
                                                    @if ($row_permission_grub->grub == '')
                                                        <h3 class="card-title">Ungrouped </h3>
                                                    @else
                                                        <h3 class="card-title">{{ $row_permission_grub->grub }}</h3>
                                                    @endif
                                                </div>
                                                <div class="card-body">
                                                    @foreach ($permission as $row_data_permission)
                                                        @if ($row_data_permission->grub == $row_permission_grub->grub)
                                                            <div class="form-group">
                                                                <div class="custom-control custom-checkbox">
                                                                    <input class="custom-control-input" type="checkbox"
                                                                        name="permission[]"
                                                                        id="customCheckbox{{ $row_data_permission->id }}"
                                                                        value="{{ $row_data_permission->id }}" @foreach ($rolePermissions as $row_rolePermissions)
                                                @if ($row_data_permission->id == $row_rolePermissions->permission_id) checked @endif @endforeach>
                                                                    <label
                                                                        for="customCheckbox{{ $row_data_permission->id }}"
